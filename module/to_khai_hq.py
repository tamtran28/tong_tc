...
def smart_date_parse(series: pd.Series) -> pd.Series:
    ...
    # ===== 1. Xử lý numeric (Excel date) =====
    numeric_values = pd.to_numeric(series_clean, errors="coerce")
    numeric_mask = numeric_values.notna()

    dates_from_number = pd.to_datetime(
    ...
    sample = series_str.dropna().head(20)
    ...
    dates_from_string = pd.to_datetime(
        series_str,
        errors="coerce",
    ...
    # ===== 3. Combine =====
    result = dates_from_string.copy()
    result[numeric_mask] = dates_from_number

    return result


# ============================================================
# 🔹 XỬ LÝ LOGIC TKHQ (FIX VECTORIZE)
# ============================================================
...
def process_tkhq_data(df: pd.DataFrame, ngay_kiem_toan: pd.Timestamp) -> pd.DataFrame:
    ...
    # ========================================================
    # (2) SỐ NGÀY QUÁ HẠN (VECTORIZE)
    # ========================================================
    mask_qua_han = (
        df["DECLARATION_DUE_DATE"].notna()
        & df["DECLARATION_RECEIVED_DATE"].isna()
        & ((ngay_kiem_toan - df["DECLARATION_DUE_DATE"]).dt.days > 0)
    )

    df["SỐ NGÀY QUÁ HẠN TKHQ"] = ""

    df.loc[mask_qua_han, "SỐ NGÀY QUÁ HẠN TKHQ"] = (
        ngay_kiem_toan - df.loc[mask_qua_han, "DECLARATION_DUE_DATE"]
    ).dt.days
    ...
    # FORMAT NGÀY
    # ========================================================
    df["DECLARATION_DUE_DATE"] = df["DECLARATION_DUE_DATE"].dt.strftime("%d/%m/%Y")
    df["DECLARATION_RECEIVED_DATE"] = df["DECLARATION_RECEIVED_DATE"].dt.strftime("%d/%m/%Y")
        ...
        with st.spinner("⏳ Đang xử lý dữ liệu..."):
            ...
            try:
                ...
                raise UserFacingError(
                    "Không thể đọc file Excel. Vui lòng kiểm tra file."
                )
